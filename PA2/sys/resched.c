...
int	resched()
{
	...
	//kprintf("\nHi, before write_back, old_pid = %d currpid=%d \n\n\n",old_pid,currpid);
	write_back();	//from frames to bs
	set_PDBR(currpid);
	//kprintf("???????????????????????? before context switch  oldpid(%d) currpid(%d) ???????????????????????????????????????\n",old_pid,currpid);
	ctxsw(&optr->pesp, optr->pirmask, &nptr->pesp, nptr->pirmask);
	//kprintf("???????????????????????? after context switch  oldpid(%d) currpid(%d) ???????????????????????????????????????\n",old_pid,currpid);
	//recover_back(currpid);	//from bs to frames.
	//kprintf("????????????????????==@@ after recover_back  oldpid(%d) currpid(%d) ???????????????????????????????????????\n",old_pid,currpid);
#ifdef	DEBUG
	PrintSaved(nptr);
#endif
