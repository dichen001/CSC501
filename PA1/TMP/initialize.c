...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	realtail = 1 + (realhead = rdyhead/2 - 1);
	//kprintf("init_real_queue, realhead= %d  .key= %d .prev= %d .next= %d\n",realhead,q[realhead].qkey, q[realhead].qprev,q[realhead].qnext);
	//kprintf("init_real_queue, realtail= %d  .key= %d .prev= %d .next= %d\n",realtail,q[realtail].qkey, q[realtail].qprev,q[realtail].qnext);
#ifdef	MEMMARK
